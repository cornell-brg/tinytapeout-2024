...
task test_encoding(input logic [5:0] key_val);
  ...
  test_character(key_val, "backspace", 1);
  test_character(key_val, "enter", 1);
endtask
...
task test_decoding(input logic [5:0] key_val);
  ...
  test_character(key_val, "backspace", 0);
  test_character(key_val, "enter", 0);
endtask
...
task test_character(input logic [5:0] key_val, string character, bit encoding);
  ...
  dut_unciphered_valid = 1;
  if (dut_unciphered_value < 6'd37) begin
    if (encoding)
      expected_val = double_alphabet[37 + digitized_char - key_val];
    else  
      expected_val = double_alphabet[digitized_char + key_val];
  end
  else
     expected_val = dut_unciphered_value;
...
initial begin
  ...
  for (int i = 0; i < 74; i++) begin
    double_alphabet[i] = i % 37; // Maps to 0-36 (A through SPACE)
  end
  ...
  test_key_value("8");
  test_key_value("9");
  test_key_value("space"); //backspace and enter cannot be keys (functional)
  $finish;
  end
