...
module Top (
  ...
  output logic       got_here1,
  output logic       got_here2
...
CaeserCipher encrypter(
  .clk(clk_input),
  .encode(encode_input), // 1 for encode, 0 for decode
  .keymode(keymode_input), // 1 for entering a key
...
LCD_display display(
  ...
  .got_to_left1(got_here1),
  .got_to_left2(got_here2)
);
